...
export class MyWorld extends GameWorld {
    Start() {
        ...
        for (let i = 0; i < 25; i++) {
            const radius = 3 + GMath.symRand(0.25);
            let polygon = GameObjectFactory.polygonGO(radius / 2, Math.round(Math.random() * 3) + 5, new ImageRendererC(Vector.zero(), undefined, undefined, 12));
        ...
        for (let i = 0; i < 25; i++) {
            let follower = GameObjectFactory.enemyGO(2.5, "Enemy nr." + i, 4, new CanonC(undefined, undefined, 10 + Math.random() * 5), new CanonRendererC(4 - 0.1), new NpcC());
