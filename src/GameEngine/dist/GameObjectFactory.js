...
export class GameObjectFactory {
    static polygonGO(radius = 2, n = 3, ...components) {
        let polygonGO = new GameObject(new RigidBodyC(0.1, 0.01), new PolygonRendererC(radius, n, 0, rgb.randomColor2().toArgb(0.75)), new BarRendererC(0.1), new ColliderC(GMath.getColliderRadius(n, radius)), new ColliderRendererC(), new HealthC(25 * n), new AnimationC(), ...components);
    ...
    static enemyGO(radius = 2.5, text = "", zindex = 4, ...components) {
        let circleGO = new GameObject(new TextRendererC(text, undefined, zindex + 0.1), new PolygonRendererC(radius, 8, zindex), new BarRendererC(zindex + 0.1), new ChasisRendererC(zindex - 0.2), new RigidBodyC(30, 0.01), new ColliderC(radius, false), new ColliderRendererC(zindex - 0.15), new HealthC(1500), new AnimationC(), new TracesRendererC(-21.37), ...components);
    ...
    static playerGO(radius = 2.5, zindex = 5, ...components) {
        ...
        new ChasisRendererC(zindex - 0.2), new PolygonRendererC(radius, 10, zindex, new rgb(50, 99, 52)), new TextRendererC("Player", true, zindex + 0.1), new ColliderRendererC(zindex - 0.15), new BarRendererC(zindex + 0.1), new CanonRendererC(zindex - 0.1), new RigidBodyC(30, 0.05), new ColliderC(radius, false), new HealthC(1000), new AnimationC(), new CanonC(6, 1.55, 44), new TracesRendererC(-21.37), ...components);
