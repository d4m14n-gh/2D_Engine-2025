...
export class CanonC extends StandaloneComponent {
    ...
    public getShotDelta(): number{
        let totalTime = this.getGameWorld()?.getWorldTime()??this.lastShootTime;
    ...
    public shoot(): void{
        ...
        if(this.getShotDelta()>=this.cooldown){
            const sW = 0.125;
            const zindex = 0;// this.getComponent(CanonRendererC).zindex-0.01;
            ...
            renderer.color = renderer.color.clone();

            // collider.avoidObjectes.add(this.getGameObject());
            
            bullet.getBody()?.setPosition(rigidBody.getPosition().add(this.getGlobalOffset()));

            let spread = this.direction.cross().mul(Math.random()*2*this.bulletSpraed-this.bulletSpraed);
            rigidBody.setVelocity(this.direction.toUnit().add(spread).mul(this.bulletSpeed));
 
            this.getPlugin(GOManagerPlugin)?.spawn(bullet);
            this.lastShootTime=this.getGameWorld()?.getWorldTime()??this.lastShootTime;
        }
    }    
} 